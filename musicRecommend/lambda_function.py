...
def get_info(trackids, like_tids):
    api_key = get_token()
...
def get_embedding_batch(tids, user=False):
    global NO_EMBEDDING
    keys = []
    visited = {}
    if len(tids) > 100:
        sample_tids = random.sample(tids, 100)
    else:
        sample_tids = tids
...
def recommendation(like_tids):
    ...
    if NO_EMBEDDING:
        return []
    right = np.concatenate([gen_candidate() for i in range(2)], axis=0).transpose()
...
def lambda_handler(event, context):
    ...
    if len(like_tids) == 0:
        return {
            "count": 0,
            "message": "Recommend: user likelist is empty, please use GetMoreRecommendation",
        }
    
    rec_idx = recommendation(like_tids.split(","))
    statusCode = 200
    if NO_EMBEDDING:
        results = {
            "count": 0,
            "message": "Recommend: The song liked is not in database, so no available embedding to run model, please use GetMoreRecommendation"
        }
        return {
    ...
    musics = get_info(rec_tid, like_tids)
    results = {
        "count": len(musics),
        "music": musics,
